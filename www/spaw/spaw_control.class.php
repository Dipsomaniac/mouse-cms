...
if (preg_match("/:\/\//i", $GLOBALS['spaw_root'])) die ("can't include external file");

include $GLOBALS['spaw_root'] . 'config/spaw_control.config.php';
include $GLOBALS['spaw_root'] . 'class/util.class.php';
include $GLOBALS['spaw_root'] . 'class/toolbars.class.php';
include $GLOBALS['spaw_root'] . 'class/lang.class.php';

// instance counter (static)
$GLOBALS['spaw_wysiwyg_instCount'] = 0;
...
class SPAW_Wysiwyg {
              ...
              $theme='', $width='100%', $height='300px', $css_stylesheet='', $dropdown_data='')
  {
    ...
    $GLOBALS['spaw_wysiwyg_instCount']++;
    
	$this->original_name = $control_name;
    $this->control_name = str_replace(']','_',str_replace('[','_',str_replace('[]','_'.$GLOBALS['spaw_wysiwyg_instCount'].'_',$control_name)));
    $this->value = $value;
    $this->width = $width;
    $this->height = $height;
    if ($css_stylesheet == '')
    {
      $this->css_stylesheet = $GLOBALS['spaw_default_css_stylesheet'];
    }
    else
    {
    ...
    if ($theme=='')
    {
      $this->theme = $GLOBALS['spaw_default_theme'];
    }
    else
    {
      $this->theme = $theme;
  ...
  function getCssScript($inline = false)
  {
    ...
    $buf = '';
    if ($GLOBALS['spaw_inline_js'])
    {
      // inline javascript
      echo "<script language='JavaScript'>\n";
      echo "<!--\n";
      echo "var spaw_active_toolbar = ".($GLOBALS['spaw_active_toolbar']?"true":"false").";\n";
      include($GLOBALS['spaw_root'].'class/script.js.php');
      echo "//-->\n";
      echo "</script>\n";
    }
    else
    {
      ...
      $buf .= "<!--\n";
      $buf .= "var spaw_active_toolbar = ".($GLOBALS['spaw_active_toolbar']?"true":"false").";\n";
      $buf .= "//-->\n";
      $buf .= "</script>\n";
      $buf .= '<script language="JavaScript" src="'.$GLOBALS['spaw_dir'].'spaw_script.js.php"></script>'."\n\n";
    }
    return $buf;
  }
  ...
  function getHtml()
  {
    ...
    {
      if ($GLOBALS['spaw_wysiwyg_instCount'] == 1)
      {
        $buf.= $this->getCssScript();
      }
      // theme based css file and javascript
      $buf.= '<script language="JavaScript" src="'.$GLOBALS['spaw_dir'].'lib/themes/'.$this->theme.'/js/toolbar.js.php"></script>';
      $buf.= '<link rel="stylesheet" type="text/css" href="'.$GLOBALS['spaw_dir'].'lib/themes/'.$this->theme.'/css/toolbar.css">';
      ...
      $buf.= '<input type="hidden" id="SPAW_'.$n.'_borders" value="on">';
  	  ...
  	  $buf.= '<iframe id="'.$n.'_rEdit" style="width:100%; height:'.$this->getHeight().'; direction:'.$this->lang->getDir().';" class="SPAW_'.$this->theme.'_editarea" frameborder="no" src="'.$GLOBALS['spaw_dir'].'empty.html"></iframe><br>';
